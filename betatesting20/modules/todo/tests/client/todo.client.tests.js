...
(function () {
  ...
  describe('Custom ToDo Angular controllers', function () {
    describe('ToDoController', function () {
      ...
      it('Initialisation must retrieve all the todo and assign to the scope variable', function () {

                //Sample response to get all todo service call
        ...
        $httpBackend.expectGET('\/api\/todo').respond(responseForInitialGetService());
        ...
        expect(scope.formData).toEqual({});
                //test if the todo list is assigned to the scope
        expect(scope.todo).toEqual(responseForInitialGetService().data);
      ...
      it('$scope.create() with valid form data should send a POST request ' +
        ...
        scope.formData.task_name = 'ToDo Task 2';
                ...
                //Initially GET call to get all toDo is called when the page is loaded
        $httpBackend.expectGET('\/api\/todo').respond(responseForInitialGetService());
        ...
        $httpBackend.expectPOST('\/api\/todo', postData()).respond(responseForPostRequest());

                //test get request called to get the final list of todo
        $httpBackend.expectGET('\/api\/todo').respond(responseForGetRequest());
        ...
        expect(scope.loading).toEqual(false);
        expect(scope.formData).toEqual({});
                //test if the response is assigned to the todo variable which displays the new list of todo
        expect(scope.todo).toEqual(responseForGetRequest().data);
      ...
      it('$scope.deleteTodo() with valid form data should send a DELETE request ' +
        ...
        $httpBackend.expectDELETE('\/api\/todo/5746d927007ed23425e27c29').respond(responseToDeleteRequest());
      ...
      it('$scope.updateToDo() with valid form data should send a PUT request ' +
        ...
        $httpBackend.expectPUT('\/api\/todo/5746d971007ed23425e27c2a', putData()).respond(responseToPutRequest());

                //test if the get request called to get the final list of todo
        $httpBackend.expectGET('\/api\/todo').respond(responseForGetRequest());
      ...
      it('$scope.toggleEdit() with valid form data should show and hide the UI elements appropriately',
                function () {
                  ...
                  var todoWhichIsToggled = {
                    '_id': '574bf050e571a8e022d4967b', 'task_name': 'To Do 2', '__v': 0,
                    'isEditable': false
                  };
      ...
      it('$scope.cancelEdit() should hide the edit views and display the old todo which was present in the UI', function () {

                //The to do object which is used for testing cancel of edit

        var todoWhichIsCancelled = {
          '_id': '574bf050e571a8e022d4967b', 'task_name': 'To Do 2', '__v': 0,
          'isEditable': true
        };
