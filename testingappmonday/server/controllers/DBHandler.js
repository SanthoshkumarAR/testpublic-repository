...
/**
 * Connect to the database by getting the DB server data as input
...
function createTable(entity,initialSchema,callBack) {
    ...
    entity.native$(function (error, connectionpool) {
        if (!error) {
            connectionpool.query(initialSchema, function (error, status) {
...
seneca.add({cmd: 'deleteObjectEvent'}, function (args, callback) {
    args.entity.remove$({'id' : args.entity.id}, function (error) {
        callback(error);
    });
});
