...
var serviceName = require('../node_starter_kit_config').service_name;
var cfMongoUrl = (function() {
  if (appEnv.getService(serviceName)) {
    var mongoCreds = appEnv.getService(serviceName).credentials;
    return mongoCreds.uri || mongoCreds.url;
  } else {
    throw new Error('No services bound to the application.');
  }
}());
